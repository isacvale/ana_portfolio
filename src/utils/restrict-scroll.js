...
const evFlipUp = new CustomEvent("flipup");
const evFlipDown = new CustomEvent("flipdown");

const flipUp = throttle(() => document.dispatchEvent(evFlipUp), 350);
const flipDown = throttle(() => document.dispatchEvent(evFlipDown), 350);

function runCallback(ev) {
  const { deltaY, key, type } = ev;
  if (type === "wheel") {
    if (deltaY > 0) flipDown();
    if (deltaY < 0) flipUp();
    return;
  }
  if (type === "keyup") {
    if (["ArrowDown", "ArrowRight", "PageDown", "End"].includes(key))
      flipDown();
    if (["ArrowUp", "ArrowLeft", "PageUp", "Home"].includes(key)) flipUp();
  }
  if (type === "keydown" && key === " ") {
    flipDown();
  }
